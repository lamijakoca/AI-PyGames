...
from scope import Sprite, Road
# from tiles import Road


class Agent(Sprite):
    def __init__(self, position, file_name):
        super(Agent, self).__init__(position, file_name, configuration.GREEN)
    ...
    def is_in_tile(self):
        return not self.rect.x % configuration.TILE_SIZE and not self.rect.y % configuration.TILE_SIZE
